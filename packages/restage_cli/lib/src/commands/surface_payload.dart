...
/// directory. Paywall-owned flow screens live under
/// `assets/paywalls/screens/`, including when another flow embeds one. Callers
/// pass [paywallScreensDir] to enable that ownership routing and may pass
/// [legacyPaywallScreensDir] to read bundles generated by an earlier SDK.
/// Passing [screensDir] as the directory the default derives yields
/// byte-identical output, so existing callers are unaffected.
///
...
Future<Uint8List> assembleSurfacePayloadBytes(
  String flowPath, {
  String? screensDir,
  String? paywallScreensDir,
  String? legacyPaywallScreensDir,
}) async {
  ...
  for (final entry in flowDocument.screenArtifacts.entries) {
    final defaultBlobPath = p.join(screensRoot, entry.value.path);
    final candidates =
        isPaywallScreenArtifact(entry.value.path) && paywallScreensDir != null
        ? <String>[
            p.join(paywallScreensDir, entry.value.path),
            if (legacyPaywallScreensDir != null)
              p.join(legacyPaywallScreensDir, entry.value.path),
          ]
        : <String>[defaultBlobPath];
    final blobPath = candidates.firstWhere(
...
///    from `paywalls/screens/`, with `onboarding/screens/` as a compatibility
///    fallback for assets generated by an earlier SDK, yielding
///    `FlowSurfacePayload.canonicalBytes`;
