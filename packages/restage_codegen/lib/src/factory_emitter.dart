...
///   * Curated `ChildrenSlot.single` entries are allowed when they have a
///     property named `'child'` of type `widget` — that property emits as
///     `child: source.child(...)` (required) or
///     `child: source.optionalChild(...)` (optional). This bullet governs only
///     canonical curated single-child handling; non-canonical and customer
///     exact `Widget` properties follow the policy-specific bullets below.
///   * Curated `ChildrenSlot.list` entries are allowed when they have a
///     property named `'children'` of type `widgetList`. That property emits
///     as `children: source.childList(...)`.
...
///   * Non-canonical `PropertyType.widget` properties (additional widget slots
///     beyond the single canonical `'child'` slot — e.g. AppBar.title,
///     Scaffold.body, IconButton.icon) preserve two intentionally distinct
///     lowering policies. Curated built-ins select `child`/`optionalChild` and
///     the corresponding cast from historical catalog requiredness, keeping
///     their generated bytes stable. Customer factories select both from
///     constructor nullability so an explicit null survives for a required
///     nullable argument. When the property carries `widgetType`, codegen
///     appends the matching nullable or non-null downcast so the value
///     type-checks against a narrower Flutter ctor parameter.
///   * Customer entries emit every exact `Widget` and `List<Widget>`
///     constructor property. Property names, declaration order, positional
...
String? emitFactoryFunction(
  ...
  bool customerChildProperties = false,
}) {
  if (!_isMechanicallyEmittable(
    entry,
    nativeIndex: nativeIndex,
    customer: customer,
    customerChildProperties: customerChildProperties,
  )) {
    return null;
  }
  ...
  // structured-type recipes, then named properties + events, then the curated
  // built-in canonical child slot last (sort_child_properties_last lint).
  ...
  for (final p in entry.properties) {
    ...
    if (customerChildProperties &&
        presence != null &&
        (p.type == PropertyType.widget || p.type == PropertyType.widgetList)) {
      positionalArguments.add(
        (
          property: p,
          value: _presenceAwareCanonicalChildrenValue(
            p,
            entry,
            presence,
            index: nativeIndex,
  ...
  for (final p in entry.properties) {
    ...
    if (customerChildProperties &&
        presence != null &&
        (p.type == PropertyType.widget || p.type == PropertyType.widgetList)) {
      final decoded = _presenceAwareCanonicalChildrenValue(
        p,
        entry,
        presence,
        index: nativeIndex,
...
String _decodeExpression(
  ...
  // broken. The generic throw fallback below applies to nullable scalar and
  // registered-structured decoders. Widget decoding follows its separate
  // customer-vs-curated policy: a required constructor-nullable customer
  // Widget argument is still present, but `optionalChild` lets its explicit
  // null survive. `childList` itself returns a non-null list; the nullable
  // no-default widget-list shape is guarded above. Event paths have their own
  // nullability contract below.
  final defaultExpr =
...
/// Whether [type]'s scalar decoder can feed the generic required-value/default
/// fallback in [_decodeExpression], where a nullable result makes a
/// `?? <fallback>` clause meaningful. Registered structured decoders are
/// recognized separately by their resolved reference.
///
/// This is intentionally not an exhaustive classification of every
/// [_decoderCallFor] emission. Widget decoding is policy-dependent and is
/// decided from `customerChildProperties` plus constructor nullability or
/// curated requiredness; in particular, a required nullable customer Widget
/// argument stays present while `optionalChild` preserves explicit null.
/// Widget lists and event callbacks are likewise handled by their dedicated
/// branches before this helper can decide a fallback.
...
String _decoderCallFor(
  ...
  switch (prop.type) {
    ...
    case PropertyType.widget:
      // Customer factories select the decoder and cast from constructor
      // nullability: a required nullable argument is still emitted
      // unconditionally, while `optionalChild` preserves an explicit null.
      // Curated factories deliberately retain their historical requiredness
      // policy so core/material/Cupertino generated bytes do not change.
      final nullableDecoder =
          customerChildProperties ? prop.constructorNullable : !prop.required;
